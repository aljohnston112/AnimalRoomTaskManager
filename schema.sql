...
SELECT SETVAL(
               PG_GET_SERIAL_SEQUENCE('animals', 'a_id'),
               COALESCE((SELECT MAX(r_id) FROM rooms), 0)
       );
...
BEGIN
    RETURN QUERY
        INSERT INTO public.censuses (date_time, r_id, u_id, a_id,
                                     number_of_animals)
            SELECT NOW(),
                   (item ->> 'r_id')::INTEGER,
                   uid,
...
CREATE OR REPLACE FUNCTION add_quantitative_task(
    task_name text,
    is_manager_only boolean,
    warning_range jsonb DEFAULT NULL,
    required_range jsonb DEFAULT NULL
    -- Expected format: {"unit": text, "minimum": real, "maximum": real}
) RETURNS int
    SET search_path TO public, auth, pg_temp AS
$$
DECLARE
    ...
    payload        jsonb;
BEGIN
    INSERT INTO public.tasks (name, manager_only, deleted)
    VALUES (task_name, is_manager_only, FALSE)
    RETURNING t_id INTO new_t_id;

    IF warning_range IS NOT NULL THEN
        INSERT INTO public.quantitative_ranges (unit, minimum, maximum, deleted)
        VALUES (warning_range ->> 'unit',
                (warning_range ->> 'minimum')::real,
                (warning_range ->> 'maximum')::real,
                FALSE)
        RETURNING qr_id INTO warning_qr_id;
    END IF;

    IF required_range IS NOT NULL THEN
        INSERT INTO public.quantitative_ranges (unit, minimum, maximum, deleted)
        VALUES (required_range ->> 'unit',
                (required_range ->> 'minimum')::real,
                (required_range ->> 'maximum')::real,
                FALSE)
        RETURNING qr_id INTO required_qr_id;
    END IF;

    INSERT INTO public.quantitative_tasks (t_id, qr_id_warning, qr_id_required)
    VALUES (new_t_id, warning_qr_id, required_qr_id);

    payload := (SELECT atv.jsonb_build_object
                FROM all_tasks_view atv
                WHERE atv.t_id = new_t_id);
    RAISE NOTICE 'payload: %', payload;
    PERFORM realtime.send(
            payload => payload::jsonb,
            event => 'task_update'::text,
            topic => 'task_channel'::text
            );
...
EXCEPTION
    WHEN unique_violation THEN
        RAISE EXCEPTION 'A quantitative task link for this ID already exists.';
    WHEN OTHERS THEN
        RAISE;
END;
$$ LANGUAGE plpgsql SECURITY INVOKER;
GRANT EXECUTE ON FUNCTION public.add_quantitative_task(text, boolean, jsonb, jsonb) TO authenticated;
...
END;
$$ LANGUAGE plpgsql SECURITY INVOKER;
GRANT EXECUTE ON FUNCTION public.add_task(text, boolean) TO authenticated;

DROP POLICY IF EXISTS "Allow sending task updates" ON realtime.messages;
CREATE POLICY "Allow sending task updates"
    ON realtime.messages
    FOR INSERT
    TO authenticated
    WITH CHECK (realtime.topic() = 'task_channel');
...
CREATE POLICY "Users can hear task updates"
    ...
    FOR SELECT
    TO authenticated
    USING ((SELECT realtime.topic()) = 'task_channel');
...
ALTER TABLE public.task_list_task_memberships
    ENABLE ROW LEVEL SECURITY;
GRANT SELECT, INSERT, DELETE ON TABLE public.task_list_task_memberships TO authenticated;
...
CREATE POLICY "TaskListTaskMembershipsInsertAuth"
    ...
    TO authenticated
    WITH CHECK (check_is_admin());
CREATE POLICY "TaskListTaskMembershipsUpdateAuth"
    ON public.task_list_task_memberships
    AS PERMISSIVE
    FOR UPDATE
    TO authenticated
    USING (check_is_admin())
    WITH CHECK (check_is_admin());
CREATE POLICY "TaskListTaskMembershipsDeleteAuth"
    ON public.task_list_task_memberships
    AS PERMISSIVE
    FOR DELETE
    TO authenticated
...
CREATE OR REPLACE FUNCTION reorder_tasks(target_tl_id int, payload jsonb)
    RETURNS void
    SET search_path TO public, auth, pg_temp AS
$$
DECLARE
...
BEGIN
    ...
    DELETE
    FROM public.task_list_task_memberships
    WHERE tl_id = target_tl_id;

    INSERT INTO task_list_task_memberships (tl_id, t_id, index)
    SELECT (new_task ->> 'tl_id')::int,
           (new_task ->> 't_id')::int,
           (new_task ->> 'new_index')::int
    ...
    payload_out := get_task_list(target_tl_id);
    PERFORM realtime.send(
...
END;
$$ LANGUAGE plpgsql SECURITY INVOKER;
GRANT EXECUTE ON FUNCTION public.reorder_tasks(int, jsonb) TO authenticated;
...
BEGIN
    ...
    SELECT JSONB_BUILD_OBJECT(
                   ...
                   't_id', tr.t_id,
                   'value', qtr.value,
                   'task_record', task_data.task_record
           )
    INTO payload
    FROM room_check_slots rcs
             LEFT JOIN rooms r ON rcs.r_id = r.r_id
             LEFT JOIN task_records tr ON tr.rc_id = rcs.rc_id
        ...
        SELECT JSONB_BUILD_OBJECT(
                       ...
                       'assigned_users', (SELECT JSONB_AGG(
                                                         JSONB_BUILD_OBJECT(
                                                                 'u_id',
                                                                 u.u_id,
                                                                 'name',
        ...
        ) AS task_data ON TRUE
    WHERE rcs.rc_id = (record_data ->> 'rc_id')::integer;
...
CREATE OR REPLACE FUNCTION get_task_records(start_date TIMESTAMP WITH TIME ZONE)
    RETURNS TABLE
            (
                result JSONB
            )
...
AS
$$
WITH task_record_data AS (SELECT tr.date_time,
                                 tr.tr_id,
                                 tr.rc_id,
                                 tr.t_id
                          ...
                          WHERE (tr.date_time >=
                                 start_date)),
     task_users AS (SELECT trd.tr_id,
                           t.t_id,
                           t.name,
                           t.manager_only,
                           JSONB_AGG(JSONB_BUILD_OBJECT(
                    ...
                    FROM task_record_data trd
                             LEFT JOIN task_record_users tru
                                       ON tru.tr_id = trd.tr_id
                             LEFT JOIN users u ON u.u_id = tru.u_id
                             LEFT JOIN tasks t ON t.t_id = trd.t_id
                    GROUP BY trd.tr_id, t.t_id, t.name,
                             t.manager_only),
     tr_grouped AS (SELECT tr.date_time,
                           rcs.r_id,
                           JSONB_AGG(JSONB_BUILD_OBJECT(
                                   'tr_id', tr.tr_id,
                                   'rc_id', tr.rc_id,
                    ...
                    FROM task_record_data tr
                             JOIN task_users tu ON tu.tr_id = tr.tr_id
                             LEFT JOIN quantitative_task_records qtr
                                       ON tr.tr_id = qtr.tr_id
                             LEFT JOIN room_check_slots rcs
                                       ON rcs.rc_id = tr.rc_id
                             LEFT JOIN quantitative_tasks qt ON tu.t_id = qt.t_id
                             LEFT JOIN quantitative_ranges qrw
                                       ON qt.qr_id_warning = qrw.qr_id
                             LEFT JOIN quantitative_ranges qrr
                                       ON qt.qr_id_required = qrr.qr_id
                    GROUP BY tr.date_time, rcs.r_id),
     records_by_date AS (SELECT r_id,
                                JSONB_AGG(JSONB_BUILD_OBJECT(
                                        'date_time', date_time,
                                        'records', records
                                          )) AS data
...
FROM records_by_date rbd
         JOIN rooms r ON r.r_id = rbd.r_id
WHERE rbd.data IS NOT NULL;
$$ LANGUAGE sql STABLE
                SECURITY INVOKER;
GRANT EXECUTE ON FUNCTION public.get_task_records(timestamptz) TO authenticated;

CREATE OR REPLACE FUNCTION get_task_records_for_room(
    rid integer,
    date timestamptz,
...
WITH task_record_data AS (SELECT tr.date_time,
                          ...
                          WHERE (tr.date_time >=
                                 date)
                            AND rcs.r_id = rid
                            AND rcs.frequency = task_frequency),
     task_users AS (SELECT trd.tr_id,
                           t.t_id,
                           t.name,
                           t.manager_only,
...
$$ LANGUAGE sql STABLE
                SECURITY INVOKER;
GRANT EXECUTE ON FUNCTION public.get_task_records_for_room(integer, timestamptz, task_frequency) TO authenticated;

CREATE OR REPLACE VIEW all_task_records_view
            WITH
            (security_invoker = on)
AS
SELECT tr.date_time AS recorded_date,
       qtr.value,
       t.name       AS task_name,
       u.name       AS user_name,
       r.name       AS room_name
FROM task_records tr
         LEFT JOIN quantitative_task_records qtr
                   ON qtr.tr_id = tr.tr_id
         LEFT JOIN tasks t ON t.t_id = tr.t_id
         LEFT JOIN task_record_users tru ON tru.tr_id = tr.tr_id
         LEFT JOIN users u ON u.u_id = tru.u_id
         LEFT JOIN room_check_slots rcs ON rcs.rc_id = tr.rc_id
         LEFT JOIN rooms r ON r.r_id = rcs.r_id;
GRANT SELECT ON TABLE public.all_task_records_view TO authenticated;